...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare("
            UPDATE tasks 
            SET title = ?, description = ?, due_date = ?, priority = ?, assigned_to = ?
            WHERE id = ?
        ");
        
        if ($stmt->execute([$title, $description, $due_date, $priority, $assigned_to, $task_id])) {
            $_SESSION['success'] = "Sarcina a fost actualizată cu succes!";
            redirect('dashboard.php');
        } else {
                ...
                <form action="" method="post">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control" id="priority" name="priority">
                            <option value="low" <?php echo $task['priority'] == 'low' ? 'selected' : ''; ?>>Scăzută</option>
                            <option value="medium" <?php echo $task['priority'] == 'medium' ? 'selected' : ''; ?>>Medie</option>
                            <option value="high" <?php echo $task['priority'] == 'high' ? 'selected' : ''; ?>>Ridicată</option>
                        </select>
                    </div>
